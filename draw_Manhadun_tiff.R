...
path <- args[1]
setwd(path)
c(Sys.glob(paste(path,"\\*.assoc",sep='')), Sys.glob(paste(path,"\\*.qassoc",sep=''))) -> file
for(i in 1 : length(file))
{
  strsplit(file[i], "\\\\") -> temp
  num <- length(temp[[1]])
  strsplit(temp[[1]][num], "\\.") -> temp
  paste(temp[[1]][1], ".tiff", sep = "") -> tiff
  fun(tiff, file[i])
}
